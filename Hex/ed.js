...
let ed = {
	Canvas : function(width, height) {
		...
		this.max = new ed.Point(this.width, this.height);
		...
		this.line = function(p1, p2, color, thickness) {
	...
	Point : function(x, y) {
		...
		this.add = function(p) {
			return new ed.Point(this.x + p.x, this.y + p.y);
		}

		this.clone = function() {
			return new ed.Point(this.x, this.y);
		}
		...
		this.randomize = function(alpha, beta, min, max) {
			...
			return new ed.Point(0, length).rotate(rotation);
		}
	},
